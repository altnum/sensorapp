...
var _ = Describe("Measurements", func() {
	...
	Context("CpuTempSensor struct", func() {
		...
		Context("SetTemperatureValue()", func() {
			It("should set temperature in positive number", func() {
				runWMICProcessElevatedMethod = cpuSensor.runWMICProcessElevated
				err := cpuSensor.SetTemperatureValue()

				Expect(err).To(BeNil())
				Expect(cpuSensor.Temperature).Should(BeNumerically(">", 0))
			})
		...
		Context("runWMICProcessElevated()", func() {
			It("given normal execution flow, then should not return error", func() {
				Getwd = os.Getwd
				err := cpuSensor.runWMICProcessElevated()

				Expect(err).To(BeNil())
			})

			It("given error, when calling os.Getwd(), then should return err", func() {
				Getwd = func() (dir string, err error) {
					return "", errors.New("")
				}
